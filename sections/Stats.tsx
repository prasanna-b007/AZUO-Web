...
const stats = [
  ...
  { label: "Client Rating", value: 4.8, suffix: "" }
];
...
function Counter({ from, to, suffix }: { from: number; to: number; suffix: string }) {
  ...
  useEffect(() => {
    if (inView) {
      ...
      const end = to;
      const duration = 2000;
      const incrementTime = Math.max(duration / Math.abs(end - start), 20);
  ...
  return (
    <span ref={nodeRef} className="text-5xl md:text-6xl font-bold text-white tracking-tighter">
      {count}
